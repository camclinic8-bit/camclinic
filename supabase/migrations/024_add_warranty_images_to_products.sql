-- Migration: 024_add_warranty_images_to_products.sql
-- Purpose: Add warranty_images TEXT[] column to job_products and update RPC functions to handle this field

-- Add warranty_images column to job_products table
ALTER TABLE job_products
ADD COLUMN IF NOT EXISTS warranty_images TEXT[] DEFAULT ARRAY[]::TEXT[];

COMMENT ON COLUMN job_products.warranty_images IS 'Base64 encoded or URL paths of compressed warranty photos for this product (maximum 5)';

-- Add spare_parts_private_details to jobs table
ALTER TABLE jobs
ADD COLUMN IF NOT EXISTS spare_parts_private_details JSONB DEFAULT '[]'::jsonb;

COMMENT ON COLUMN jobs.spare_parts_private_details IS 'Private list of spare parts used with name, quantity, and unit cost for internal office use only';
...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname IN ('create_job_with_products', 'update_job_with_products', 'sync_job_products')
    LOOP
...
-- ============================================================================
-- 3. Recreate RPC function: sync_job_products
-- ============================================================================
