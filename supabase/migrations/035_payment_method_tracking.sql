-- Migration: 035_payment_method_tracking.sql
-- Purpose: Track how each payment was collected — cash, UPI, card, or bank
-- transfer. Constrain payment_transactions.payment_method to the allowed
-- values and record a payment transaction for advance amounts collected at
-- job creation / edit time (previously only jobs.advance_paid was updated
-- without any per-transaction record).

-- 1. Normalize any legacy values, then constrain to the supported methods.
UPDATE payment_transactions
SET payment_method = 'cash'
WHERE payment_method IS NULL
   OR payment_method NOT IN ('cash', 'upi', 'card', 'bank_transfer');

ALTER TABLE payment_transactions DROP CONSTRAINT IF EXISTS payment_method_allowed;
ALTER TABLE payment_transactions
  ADD CONSTRAINT payment_method_allowed
  CHECK (payment_method IN ('cash', 'upi', 'card', 'bank_transfer'));

COMMENT ON COLUMN payment_transactions.payment_method IS 'Payment method: cash, upi, card, or bank_transfer';

-- 2. Recreate the job RPCs with advance payment method support.
--    Drop all potential old overloads to prevent PostgreSQL signature clashes.
DO $$
DECLARE
    r RECORD;
BEGIN
...
-- Now records a payment_transactions row when an advance is collected at
-- job creation, using p_advance_payment_method.
-- ============================================================================
CREATE OR REPLACE FUNCTION create_job_with_products(
  ...
  p_advance_payment_method TEXT DEFAULT 'cash'
)
RETURNS JSONB AS $$
DECLARE
  v_job_number TEXT;
...
BEGIN
  ...
  v_method := COALESCE(NULLIF(TRIM(p_advance_payment_method), ''), 'cash');
  IF v_method NOT IN ('cash', 'upi', 'card', 'bank_transfer') THEN
    RAISE EXCEPTION 'Invalid advance payment method: %', v_method;
  END IF;
  ...
  -- Record the advance collected at creation as a tracked payment transaction.
  IF COALESCE(p_advance_paid, 0) > 0 THEN
    INSERT INTO payment_transactions (
      ...
      p_advance_paid,
      v_method,
      'Advance collected at job creation',
      p_created_by
    );
  END IF;
...
-- When the advance amount is increased, the increase is recorded as a payment
-- transaction using p_advance_payment_method so payment history stays complete.
-- ============================================================================
...
DECLARE
  ...
  v_result JSONB;
BEGIN
  SELECT status, advance_paid INTO v_current_status, v_old_advance
  FROM jobs
  WHERE id = p_job_id;
  ...
  -- Any increase in the advance amount is money collected now: record it.
  IF p_advance_paid IS NOT NULL AND p_advance_paid > COALESCE(v_old_advance, 0) THEN
    INSERT INTO payment_transactions (
      job_id,
      amount,
      payment_method,
      ...
      p_job_id,
      p_advance_paid - COALESCE(v_old_advance, 0),
      v_method,
      'Payment collected via job edit',
      p_user_id
    );
  END IF;
